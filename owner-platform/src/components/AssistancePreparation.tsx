...
import { createAssistanceProposal, listAssistanceSnapshots, parseAssistancePatch, type AssistanceSnapshot } from '@/assist/client'
      ...
      {loading ? <p role="status">Cargando snapshots…</p> : snapshots.length === 0 ? <p>No hay snapshots de previsualización disponibles.</p> : (
        <form onSubmit={submit}>
          <div className={styles.proposal}>
            <label><span>Snapshot verificado</span><select name="snapshot" required>{snapshots.map((snapshot, index) => <option value={index} key={snapshot.id}>{snapshot.label}</option>)}</select></label>
            <label><span>Patch JSON</span><textarea name="patch" defaultValue={example} rows={12} required spellCheck={false} /></label>
