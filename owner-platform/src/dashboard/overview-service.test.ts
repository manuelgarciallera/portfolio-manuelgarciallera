...
describe('getOwnerDashboardOverview', () => {
  it('loads the three owner dashboard domains in parallel', async () => {
    ...
    const readiness = vi.fn(async () => ({ productionReady: false }))
    const overview = await getOwnerDashboardOverview({ activity, analytics, content, integrations, media, readiness, recent, releases, user: owner, workflow })
    expect(overview).toEqual({ activity: { count: 1, events: [] }, analytics: { available: true, data: { traffic: { visitors: 10 } } }, content: { issueCount: 2 }, integrations: { connectors: {} }, media: { issueCount: 1 }, readiness: { productionReady: false }, recent: { projects: [] }, releases: { count: 3, versions: [] }, workflow: { attentionCount: 2 } })
    ...
    expect(readiness).toHaveBeenCalledWith(owner)
  })

  it('represents absent analytics without hiding integrity failures', async () => {
    const base = { activity: async () => ({}), content: async () => ({}), integrations: async () => ({}), media: async () => ({}), readiness: async () => ({}), recent: async () => ({}), releases: async () => ({}), workflow: async () => ({}) }
  ...
  it('rejects anonymous access before loading dashboard data', async () => {
    const content = vi.fn()
    await expect(getOwnerDashboardOverview({ activity: vi.fn(), analytics: vi.fn(), content, integrations: vi.fn(), media: vi.fn(), readiness: vi.fn(), recent: vi.fn(), releases: vi.fn(), user: null, workflow: vi.fn() })).rejects.toThrow(/owner/i)
