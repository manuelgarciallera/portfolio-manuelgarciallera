...
describe('getOwnerDashboardOverview', () => {
  it('loads the three owner dashboard domains in parallel', async () => {
    ...
    const overview = await getOwnerDashboardOverview({ activity, analytics, content, releases, user: owner })
    expect(overview).toEqual({ activity: { count: 1, events: [] }, analytics: { available: true, data: { traffic: { visitors: 10 } } }, content: { issueCount: 2 }, releases: { count: 3, versions: [] } })
  ...
  it('represents absent analytics without hiding integrity failures', async () => {
    const base = { activity: async () => ({}), content: async () => ({}), releases: async () => ({}) }
  ...
  it('rejects anonymous access before loading dashboard data', async () => {
    const content = vi.fn()
    await expect(getOwnerDashboardOverview({ activity: vi.fn(), analytics: vi.fn(), content, releases: vi.fn(), user: null })).rejects.toThrow(/owner/i)
