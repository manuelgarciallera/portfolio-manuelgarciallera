...
describe('getOwnerDashboardOverview', () => {
  it('loads the three owner dashboard domains in parallel', async () => {
    ...
    const overview = await getOwnerDashboardOverview({ activity, analytics, content, integrations, releases, user: owner, workflow })
    expect(overview).toEqual({ activity: { count: 1, events: [] }, analytics: { available: true, data: { traffic: { visitors: 10 } } }, content: { issueCount: 2 }, integrations: { connectors: {} }, releases: { count: 3, versions: [] }, workflow: { attentionCount: 2 } })
  ...
  it('represents absent analytics without hiding integrity failures', async () => {
    const base = { activity: async () => ({}), content: async () => ({}), integrations: async () => ({}), releases: async () => ({}), workflow: async () => ({}) }
  ...
  it('rejects anonymous access before loading dashboard data', async () => {
    const content = vi.fn()
    await expect(getOwnerDashboardOverview({ activity: vi.fn(), analytics: vi.fn(), content, integrations: vi.fn(), releases: vi.fn(), user: null, workflow: vi.fn() })).rejects.toThrow(/owner/i)
