...
export const getOwnerWorkflowSummary = async ({ payload, req }: { payload: CountPayload; req: { user?: unknown } }) => {
  ...
  const [pending, accepted, rejected, ready, confirmed, conflict, executed, bundles, approved, reviewRejected, artifacts, figmaImportPlans, figmaImportReviews] = await Promise.all([
    ...
    run('figma-import-plans'), run('figma-import-reviews'),
  ])
  return buildWorkflowSummary({ artifacts, bundles, figmaImportPlans, figmaImportReviews, proposals: { accepted, pending, rejected }, restores: { confirmed, conflict, executed, ready }, reviews: { approved, rejected: reviewRejected } })
}
