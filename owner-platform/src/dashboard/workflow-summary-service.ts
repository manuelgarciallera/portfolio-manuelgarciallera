...
export const getOwnerWorkflowSummary = async ({ payload, req }: { payload: CountPayload; req: { user?: unknown } }) => {
  ...
  const [pending, accepted, rejected, ready, confirmed, conflict, executed, bundles, approved, reviewRejected, artifacts] = await Promise.all([
  ...
  return buildWorkflowSummary({ artifacts, bundles, proposals: { accepted, pending, rejected }, restores: { confirmed, conflict, executed, ready }, reviews: { approved, rejected: reviewRejected } })
}
