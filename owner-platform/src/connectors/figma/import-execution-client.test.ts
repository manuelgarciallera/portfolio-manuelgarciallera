...
describe('executeFigmaImport', () => {
  it('submits the exact bounded execution envelope and returns safe owner destinations', async () => {
    const request = vi.fn(async () => new Response(JSON.stringify({ execution: { id: 73, media: 72 } }), { status: 201 }))
    await expect(executeFigmaImport(45, { alt: ' Vista principal ', confirmation: 'IMPORTAR PNG DE FIGMA' }, request)).resolves.toEqual({ executionHref: '/admin/collections/figma-import-executions/73', mediaHref: '/admin/collections/media/72' })
