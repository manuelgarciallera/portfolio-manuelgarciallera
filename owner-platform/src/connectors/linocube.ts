...
export const validatePublishedManifest = (input: unknown): PublishedManifest => {
  ...
  const storedManifest = input.manifest
  exactKeys(storedManifest, ['schemaVersion', 'source', 'brandTokens', 'pageBlocks', 'mediaReferences', 'hash', ...['pageTitle', 'mediaPlacements'].filter((key) => Object.hasOwn(storedManifest, key))], 'El manifiesto')
  ...
  if (Object.hasOwn(input.manifest, 'mediaPlacements')) assertBoundedPlainData(input.manifest.mediaPlacements)
  const projected = createPreviewManifest({ source: input.manifest.source as PreviewManifest['source'], pageTitle: input.manifest.pageTitle as string | undefined, mediaPlacements: input.manifest.mediaPlacements as PreviewManifest['mediaPlacements'], brandTokens: input.manifest.brandTokens, pageBlocks: input.manifest.pageBlocks, mediaReferences: input.manifest.mediaReferences })
