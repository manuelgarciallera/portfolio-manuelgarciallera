...
export async function copyMigrationRevisions(input: Input) {
  const { serializedPlan, serializedInventory, expectedInventoryHash, sourceRoot, destination } = input
  ...
  return { schemaVersion: 1 as const, status: 'copied-and-verified' as const, canApply: false as const,
    planDigest: plan.digest, inventoryHash: expectedInventoryHash, revisions, fileCount, totalBytes }
}
