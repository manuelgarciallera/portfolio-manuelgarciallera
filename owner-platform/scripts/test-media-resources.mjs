...
if (availableBytes < minimumAvailableBytes) {
  ...
  try {
    ...
    fixture = await startMediaHTTPFixture({ root, revisionRoot: path.join(root, 'revisions'), staticDir: path.join(root, 'unused-native-media'),
    ...
    try { if (fixture) { await fixture.close(); serverClosed = true } }
    ...
    result.serverClosed = serverClosed
    result.finishedAt = new Date().toISOString()
    try {
      assert(workersClosed() && serverClosed, 'Cleanup requires actual client and server shutdown')
      await assertTaskRoot(cache, root, prefix)
      const files = []
      const directories = []
