...
export const verifyBrowserMediaPlacement = async ({ page, origin, width }) => {
  ...
  assert.equal(await editor.getByRole('button', { name: 'Usar encuadre de escritorio', exact: true }).isDisabled(), true)
  ...
  assert.match(await editor.innerText(), /ajustes propios/)
  await reset.press('Enter')
  assert.equal(await zoom.inputValue(), '1')
  assert.equal(await vertical.inputValue(), '50')
  assert.equal(await reset.isDisabled(), true)
  const resetSaved = page.waitForResponse(response => response.request().method() === 'PATCH' && new URL(response.url()).pathname === `/api/media-placements/${placement.id}`)
  await page.locator('#action-save-draft').press('Enter')
  assert.equal((await resetSaved).status(), 200)
  await page.reload({ waitUntil: 'domcontentloaded' })
  await page.locator('form[data-form-ready="true"]').first().waitFor()
  await editor.getByRole('button', { name: 'Mobile', exact: true }).click()
  assert.equal(await zoom.inputValue(), '1', 'Cleared mobile values must stay inherited after saving and reopening')
  assert.equal(await vertical.inputValue(), '50')
  assert.equal(await reset.isDisabled(), true)
  assert.match(await editor.innerText(), /Usa el encuadre de escritorio/)
  // Keep the downstream page-preview assertion for a deliberately customized
  // crop as well; resetting must not prevent editing this format again.
  await vertical.press('Home')
  await zoom.press('End')
  await editor.getByRole('combobox', { name: 'Proporción', exact: true }).selectOption('1:1')
  const customizedAgain = page.waitForResponse(response => response.request().method() === 'PATCH' && new URL(response.url()).pathname === `/api/media-placements/${placement.id}`)
  await page.locator('#action-save-draft').press('Enter')
  assert.equal((await customizedAgain).status(), 200)
