...
try {
  for (const width of [320, 390, 768, 1280]) for (const theme of ['light', 'dark']) {
    ...
    try {
      ...
      const analyticsLink = page.getByRole('link', { name: /Abrir estadísticas en Umami/ })
      assert.equal(await analyticsLink.count(), 1, 'External analytics must be available even without imported snapshots')
      assert.equal(await analyticsLink.getAttribute('href'), 'https://cloud.umami.is/analytics/eu/websites/7c0010a4-8f44-4340-8ee2-d8a7bda1152c')
      assert.equal(await analyticsLink.getAttribute('target'), '_blank')
      assert.match(await analyticsLink.getAttribute('rel'), /noreferrer/)
      const analyticsBox = await analyticsLink.boundingBox()
      assert.ok(analyticsBox && analyticsBox.x >= 0 && analyticsBox.x + analyticsBox.width <= width)
      ...
      assert.equal(await analyticsLink.count(), 1, 'Provider access must remain separate from imported metrics')
