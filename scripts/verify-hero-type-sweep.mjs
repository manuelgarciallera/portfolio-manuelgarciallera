...
try {
  for (const width of [390, 1280]) for (const theme of ['dark', 'light']) {
    ...
    assert.equal(parseFloat(rest.position), 60, 'color already lies across the headline at first frame')
    ...
    const after = await sample(800)
    assert.equal(parseFloat(after.position), 0, 'first sweep completes in 0.8 seconds')
    assert.equal(parseFloat(after.y), 0)
    assert.equal((await sample(2000)).position, after.position, 'solid text after the short pass')
    assert.equal((await sample(19000)).position, after.position, 'long quiet interval')
    assert.equal((await sample(20600)).position, mid.position, 'next sweep repeats after twenty seconds')
