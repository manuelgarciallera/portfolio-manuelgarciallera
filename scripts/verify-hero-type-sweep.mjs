...
try {
  for (const width of [390, 1280]) for (const theme of ['dark', 'light']) {
    ...
    const after = await sample(1400)
    assert.ok(Math.abs(parseFloat(after.position)) < .001, 'first sweep completes in 1.4 seconds')
    ...
    assert.ok(Math.abs(parseFloat((await sample(19000)).position)) < .001, 'long quiet interval')
