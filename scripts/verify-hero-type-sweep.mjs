...
try {
  for (const width of [390, 1280]) for (const theme of ['dark', 'light']) {
    ...
    assert.equal(rest.duration, width<768?18000:10000)
    ...
    const after = await sample(width<768?3600:4000)
    assert.ok(Math.abs(parseFloat(after.position)) < .001, 'mobile completes in3.6s; desktop in4s')
    ...
    assert.ok(Math.abs(parseFloat((await sample(width<768?17000:9000)).position)) < .001, 'quiet interval')
    assert.equal((await sample(width<768?18600:10600)).position, mid.position, 'next sweep repeats at its viewport cadence')
