...
try {
  for(const width of [390,900]) {
    ...
    await page.getByRole('button',{name:'Ondas de color',exact:true}).click({timeout:3000})
    const waves=await canvas.screenshot()
    assert.notDeepEqual(waves,frozen,'color waves change the rendered pigment')
    assert.deepEqual(await canvas.screenshot(),waves,'color variant respects reduced motion')
    await page.screenshot({path:`.audit/organic-orb/${width}-waves-dark.png`})
    await page.getByRole('button',{name:'Azul original',exact:true}).click()
    assert.deepEqual(await canvas.screenshot(),frozen,'approved blue appearance is exactly recoverable')
    ...
    await page.getByRole('button',{name:'Ondas de color',exact:true}).click()
    await page.screenshot({path:`.audit/organic-orb/${width}-waves-light.png`})
    ...
    console.log('PASS local prototype',width,'shader, both palettes, exact blue restore, themes, reduced motion, wave play/pause, overflow')
    await page.close()
  }
