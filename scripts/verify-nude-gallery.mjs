...
try {
  for (const width of [360, 768, 1440]) {
    ...
    for (const theme of ['light', 'dark']) {
      ...
      assert.equal(await links.last().getAttribute('href'), '/proyectos/nude-project')
      ...
      await page.goto(`${base}/proyectos/nude-project`, { waitUntil: 'networkidle' })
      await page.evaluate(value => document.documentElement.setAttribute('data-theme', value), theme)
      ...
      assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1))
      assert.equal(await page.locator('.rd-next-case a[href="/proyectos/buy-sell-marketplace"]').count() > 0, true)
