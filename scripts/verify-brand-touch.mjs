...
try {
  ...
  const color = page.locator('.rd-brand-m')
  ...
  const response = await signature.evaluate(async e => {
    ...
    return {
      gradient: getComputedStyle(e.querySelector('.rd-brand-m'), '::after').animationName,
  ...
  assert.equal(response.gradient, 'rd-brand-touch-reflection', 'touch starts a translucent reflection')
