...
try {
 for(const width of [320,390,430,767]) {
  ...
  const previousFont=Math.min(28,Math.max(16,width*.06))
  const old=await page.addStyleTag({content:`.rd-hero-art{transform:none!important}.rd-hero-name{font-size:${previousFont}px!important;transition:none!important}`})
  await page.waitForFunction(size=>Math.abs(parseFloat(getComputedStyle(document.querySelector('.rd-hero-name')).fontSize)-size)<.02,previousFont)
