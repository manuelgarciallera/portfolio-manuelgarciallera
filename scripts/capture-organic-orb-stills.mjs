...
try{
  for(const compact of [false,true])for(const theme of ['dark','light']){
    ...
    source=source.replace('gl.drawArrays(gl.TRIANGLES,0,6);',`gl.uniform1f(gl.getUniformLocation(program,'cameraZoom'),${compact?1.5:2.6});gl.uniform1f(gl.getUniformLocation(program,'verticalOffset'),${compact?.36:0});gl.drawArrays(gl.TRIANGLES,0,6);`)
    ...
    const path=`public/art/hero-organic-static-${compact?'mobile-':''}${theme}.png`
