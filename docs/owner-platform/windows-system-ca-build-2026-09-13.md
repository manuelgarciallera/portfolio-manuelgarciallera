# Windows build: system CA worker incompatibility

Base `fb5e428`, Windows Node24.13.0, Next16.3.4. This is a diagnosed build-tool incompatibility and a one-shot workaround candidate, not a fix to Node/Next or a permanent build configuration change. Compilation passed but process termination is still pending; do not call the whole command successful.

## Reproduction

Ordinary owner build fails before compilation with `ERR_WORKER_INVALID_EXEC_ARGV`: `--use-system-ca` is not allowed in a worker's NODE_OPTIONS (`2363fa`). Node reports that the option is allowed in its environment. A basic Worker with inherited arguments succeeds (`0f0b6e`); a worker with explicit empty execArgv and the same system-CA option fails (`e63439`). Next's real option parser plus explicit empty execArgv reproduces the same rejection (`1f173f`). An initial `node -e` probe accidentally included the eval argument in parsed options and is not evidence for the CA diagnosis (`4aef6a`).

Installed source confirms `next/dist/build/turbopack-build/index.js` creates a thread worker, and `next/dist/lib/worker.js` explicitly supplies execArgv. `next/dist/build/index.js` honors `NEXT_TURBOPACK_USE_WORKER=0` to run the same Turbopack implementation in the main process. This switch is internal, not presented as a documented stable public API.

## Temporary invocation under observation

From owner-platform, without changing machine-wide environment:

```text
node -e "const result=require('node:child_process').spawnSync(process.execPath,['scripts/build.mjs'],{stdio:'inherit',env:{...process.env,NEXT_TELEMETRY_DISABLED:'1',NEXT_TURBOPACK_USE_WORKER:'0'}});if(result.error)throw result.error;process.exit(result.status??1)"
```

Session86741 compiles in10.2s, passes TypeScript, generates23/23 static pages and route output (`30081a`, `b0f5da`). NODE_OPTIONS, system trust, antivirus and certificate verification remain unchanged. Same bundler, no webpack substitution or upstream patch. This does not prove byte-identical build output or fix unrelated Windows native SQLite intermittency.

Use only as an explicit local workaround for this pinned combination. Do not silently add it to deployment or global settings. Revalidate after Node/Next updates; remove the workaround when the ordinary build succeeds under the same trust configuration. No runtime source, dependencies, accounts, database, public design or deployment changed in this investigation.

At `b542a7` the same session86741 remains live. Process inventory `bced71` confirms owned chain62236→67464→58988 and worker children; older unrelated processes are untouched. No exit code is available yet. Continue observing this handle, do not start a duplicate build or infer completion from route output. This qualification supersedes any wording suggesting a finished workaround.

## Final disposition: candidate rejected

The same handle remained live beyond five minutes after starting (`a84ef6`, `4eaf9e`). Installed Next build source performs route statistics, telemetry flush and awaits `shutdownPromise` after printing the route tree, before ending its static workers. Therefore route output alone cannot locate the precise pending operation; a Turbopack shutdown hang remains a hypothesis, not a proven cause.

The diagnostic build was stopped after validating PID58988, parent67464, creation-time interval and Next build command. Only its seven worker children and that owned build process were terminated (`25e009`); wrapper session86741 then returned exit1 (`9d406a`). No files were deleted and unrelated Node processes were untouched. An initial exact timestamp guard refused the stop because the displayed timestamp omitted fractional seconds (`43033b`); the subsequent guard checked the original one-second interval as well as parent and command.

Do not use `NEXT_TURBOPACK_USE_WORKER=0` as an accepted workaround. The normal Windows build and this candidate both remain failed gates, for different reasons. Adding explicit `--use-system-ca` to worker execArgv while preserving it in NODE_OPTIONS also reproduces the original rejection (`bffcf2`), so that is not a fix either. No certificate verification or system trust was changed. Next step: validate the minimal worker reproduction against an isolated supported Node runtime before another full Windows build; retain the separately successful Linux build evidence without claiming Windows parity.

The already-installed bundled Node24.19.0 executable reproduces the same minimal worker failure (`7a6f4e`), without installation or changing PATH. Updating from system Node24.13.0 to that executable alone is therefore not an established solution. No second full build was started. Owned build processes were confirmed absent (`3fc349`). Continue with existing CMS functional gates while keeping this Windows build gate open; no certificate bypass or speculative upstream patch.
