...
describe('analytics consent controller', () => {
  ...
  it('an asynchronous SDK failure stops only that provider and stays visible', () => {
    const { controller, events } = setup()
    controller.choose({ google: true, umami: true })
    controller.reportFailure('google')
    controller.visit('/casos')
    expect(controller.getSnapshot().error).toBe('provider')
    expect(events).toEqual(['google:start', 'umami:start', 'google:stop', 'umami:/casos'])
    controller.refresh()
    expect(controller.getSnapshot().error).toBe('provider')
    expect(events.filter(event => event === 'google:start')).toHaveLength(1)
  })
})
