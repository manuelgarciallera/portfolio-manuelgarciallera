...
const IcoPlay=({s=20,c="#fff"})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none"><path d="M8 6.8L17.2 12 8 17.2V6.8z" stroke={c} strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"/></svg>;
    ...
    <section style={{padding:"132px 0 0",background:isDark?"#1c1c24":"#f0f0f3",transition:"background .5s",minHeight:"84vh"}}>
      ...
      <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:18,marginTop:72}}>
          ...
          <button onClick={()=>setPlaying(v=>!v)} aria-label={playing?"Pausar galeria":"Reproducir galeria"}
            ...
            {playing?<IcoPause s={20} c="#f5f5f7"/>:<IcoPlay s={20} c="#f5f5f7"/>}
          </button>
        </div>
    </section>
  );
          ...
          <div style={{position:"relative",minHeight:wide?800:880}}>
            ...
            {open!==-1&&(
              <button onClick={()=>setOpen(-1)} aria-label="Cerrar descripcion"
                ...
                style={{position:"absolute",top:26,right:26,zIndex:15,width:ctrlSize,height:ctrlSize,borderRadius:"50%",border:"none",cursor:"pointer",fontSize:25,fontWeight:760,lineHeight:1,background:xHover?"rgba(24,24,30,.96)":"rgba(42,42,48,.9)",color:"#d8d8df",transition:"background .24s ease"}}>
                {"\u00D7"}
              </button>
            )}
