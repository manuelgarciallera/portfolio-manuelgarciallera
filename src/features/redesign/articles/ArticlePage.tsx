...
export function ArticlePage({ article }: { article: Article }) {
  ...
  return <main className="rd-article-page" id="main-content">
    <header><Breadcrumbs items={[{ href: PUBLIC_ROUTES.blog, label: 'Blog' }, { label: article.title }]} /><p>{article.category}</p><h1>{article.title}</h1><strong>{article.summary}</strong><span>Por {ARTICLE_AUTHOR.name} · {article.readTime} de lectura</span></header>
    ...
    <aside className="rd-more-articles" aria-labelledby="more-articles-title">
      <header><p>Más en el blog</p><h2 id="more-articles-title">Una idea abre la siguiente.</h2></header>
      <div>{relatedArticles.map((related) => <article key={related.slug}>
        <Link href={articleHref(related.slug)} aria-label={`Artículo ${related.index}: ${related.title}`}><ArticleCover theme={related.theme} index={related.index} /></Link>
        <p>{related.category}</p>
        <h3><Link href={articleHref(related.slug)}>{related.title}</Link></h3>
        <span>{related.readTime} de lectura</span>
      </article>)}</div>
    </aside>
    <nav className="rd-article-next"><span>Sigue leyendo</span><Link href={PUBLIC_ROUTES.blog}>Explorar el blog →</Link></nav>
  </main>
}
