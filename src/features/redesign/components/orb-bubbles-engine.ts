export interface LetterBox { left: number; right: number; top: number; bottom: number }
interface Bubble { x: number; y: number; vx: number; vy: number; radius: number; born: number; life: number; hit: number | null; alpha: number; hue: number }
export function createBubbleEmitter(random = Math.random) {
  const particles: Bubble[] = []
  let previous: { x: number; y: number; now: number; inside: boolean } | null = null
  let lastBurst = -Infinity, lastTick: number | null = null
  return {
    particles,
    clear() { particles.length = 0; previous = null; lastTick = null; lastBurst = -Infinity },
    move(x: number, y: number, now: number, inside: boolean) {
      ...
      for (let i = 0; i < 5 && particles.length < 28; i++) {
        ...
        particles.push({ x: (inside ? x : prior.x) - Math.cos(direction) * i * 10, y: (inside ? y : prior.y) - Math.sin(direction) * i * 10 + (random() - .5) * 18,
          vx: Math.cos(angle) * velocity, vy: Math.sin(angle) * velocity, radius: 3 + random() * 6,
          born: now, life: 1900 + random() * 650, hit: null, alpha: 0, hue: 24 + random() * 16 })
      }
      if (lastTick === null) lastTick = now
    },
    tick(now: number, letters: LetterBox[] = []) {
